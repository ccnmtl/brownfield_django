...
var ManageCoursesView = Backbone.View.extend({
    ...
    showCourseForm: function(e) {
        ...
        this.user_list.forEach(function(model) {
            ...
            jQuery('#id_professor')
                .append("<option value='" + String(model.attributes.id) + "'>" + 
    ...
    validateForm: function() {
        ...
        if(course_name === null || course_name === "") {
            ...
            if((jQuery('.course-name-block').has('.error-msg').length) === 0) {
                jQuery('.course-name-block')
                    .append("<p class='error-msg' style='color:#ff0000'>Please enter a valid course name.</p>");
            }
        }
        if(startingBudget === null || startingBudget === "") {
            ...
            if((jQuery('.course-budget-block').has('.error-msg').length) === 0) {
                jQuery('.course-budget-block')
                    .append("<p class='error-msg' style='color:#ff0000'>Please enter a valid starting budget for your course.</p>");
            }
        }
                ...
                error: function(model, response) {
                    if((jQuery("#create-course-form")
                        ...
                        jQuery("#create-course-form")
                            .append("<p class='error-msg form-error'>Something went wrong, please try again.</p>");
                    }
                },
            wait: true
        });
...
var ManageInstructorsView = BaseManagementView.extend({
    ...
    validAddForm: function() {
        ...
        if(this.is_empty(".add-instructor-frm input.instructor-frst-name",
                         ".inst-first-name", "Please enter a first name.")) {
            is_valid = false;
        }
        if(this.is_empty(".add-instructor-frm input.instructor-last-name",
                         ".inst-last-name", "Please enter a last name.")) {
            is_valid = false;
        }
        if(this.is_empty(".add-instructor-frm input.instructor-email",
            ...
            if((jQuery(".add-instructor-frm input.instructor-email")
               ...
               (jQuery(".add-instructor-frm input.instructor-email")
                ...
                if((jQuery(".inst-email").has('b').length) === 0) {
                    jQuery(".inst-email")
                        .append("<b class='error-msg' style='color:red'>Please enter a valid email.</b>");
                }
            }
        }
        return is_valid;
...
var StudentControlView = BaseManagementView.extend({
    ...
    validAddForm: function() {
        ...
        if(this.is_empty(".add-std-frm input.frst-name",
                         ".first-name-box", "Please enter a first name.")) {
            is_valid = false;
        }
        if(this.is_empty(".add-std-frm input.last-name",
                         ".last-name-box", "Please enter a last name.")) {
            is_valid = false;
        }
        if(this.is_empty(".add-std-frm input.email",
                         ".email-box", "Please enter a email.")) {
            is_valid = false;
        }
        //check whatever they put for email looks something like an actual address
        else if((jQuery(".add-std-frm input.email").val().length) !== 0) {
            if((jQuery(".add-std-frm input.email").val()
                .indexOf("@")  === -1) &&
...
var TeamControlView = BaseManagementView.extend({
    ...
    validAddForm: function() {
        ...
        if((jQuery(".add-team-frm input.team-name").val().length) === 0)
        {
            ...
            {
                jQuery(".team-name-box")
                    .append("<b class='error-msg' style='color:red'>Please enter a team name.</b>");
            }
        }
        return is_valid;
    },
