...
class TestStudentRestViews(APITestCase):
    ...
    def setUp(self):
        ...
        self.student_one = UserProfileFactory(
            user=UserFactory(username='student_one', first_name='astudent',
    ...
    def test_create_student_as_admin(self):
        ...
        self.client.login(username=self.admin.user.username, password="test")
        response = self.client.post('/api/student/?course=' + str(self.course.pk),
    ...
    def test_get_students_as_teacher(self):
        ...
        self.assertEqual(len(response.data), 4)
        self.assertEqual(response.data[0]['first_name'], self.student_one.user.first_name)
        self.assertEqual(response.data[1]['first_name'], self.student_two.user.first_name)
        self.assertEqual(response.data[2]['first_name'], self.student_three.user.first_name)
        self.assertEqual(response.data[3]['first_name'], self.student_four.user.first_name)
    ...
    def test_update_student_as_admin(self):
        ...
        response = self.client.get('/api/student/?course=' +
                                   str(self.populated_course.pk), format='json')
        new_response = self.client.put('/api/student/'+ str(response.data[0]['id']) + '/',
